...
| 7 | OpenAI SDK against any compatible base URL | Provider-specific SDK, or a custom abstraction layer | "LLM-agnostic" via the interface most providers already speak. Switching is env vars (base URL, key, chat model, embedding model — see `backend/env.sample`); no adapter layer to maintain. |
